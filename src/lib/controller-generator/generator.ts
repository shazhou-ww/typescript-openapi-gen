...
import { generateIndexFile } from './index-file-generator.js'
...
export class ControllerGenerator {
  ...
  constructor(doc: OpenAPIDocument, outputDir: string) {
    this.doc = doc
  ...
  private createDirectoryIfNeeded(controllerDir: string, info: RouteInfo): void {
  ...
  private async generateControllerIfNeeded(
    ...
    for (const [method, operation] of info.methods) {
      generateMethodFileIfNotExists(controllerDir, method, operation, this.result)
    }
  }
}
