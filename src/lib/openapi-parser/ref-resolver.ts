...
async function resolveExternalRef(
  ...
  try {
    ...
    let refDoc: any
    try {
      // Import parseOpenAPIFile dynamically to avoid circular dependency
      const parseModule = await import('./parse.js')
      refDoc = await parseModule.parseOpenAPIFile(fullPath)
    } catch {
