...
function registerFastifyCommand(program: Command) {
  program
    .command('fastify')
    .description('Generate Fastify app (controllers + routes) from OpenAPI specification')
    ...
    .option('--route-file <name>', 'Route file name (default: fastify-routes.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routeFile = options.routeFile ?? 'fastify-routes.ts'
      ...
      try {
        ...
        const routeGenerator = new FastifyRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
