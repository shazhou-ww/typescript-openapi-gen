...
function registerElysiaCommand(program: Command) {
  program
    .command('elysia')
    .description('Generate Elysia app (controllers + routes) from OpenAPI specification')
    ...
    .option('--route-file <name>', 'Route file name (default: elysia-routes.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routeFile = options.routeFile ?? 'elysia-routes.ts'
      ...
      try {
        ...
        const routeGenerator = new ElysiaRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
