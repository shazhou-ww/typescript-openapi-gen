...
export default class GenElysia extends Command {
  ...
  static override description =
    'Generate Elysia app (controllers + routes) from OpenAPI specification'
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(GenElysia)
    ...
    try {
      ...
      const routeGenerator = new ElysiaRouteGenerator(
        openApiDoc,
        controllerPath,
        outputPath,
        { prettierConfig },
