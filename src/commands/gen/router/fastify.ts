...
import { FastifyRouteGenerator } from '../../../lib/route-generator'

function registerRouterFastifyCommand(program: Command) {
  program
    .command('fastify')
    .description('Generate Fastify router decorator from OpenAPI specification')
    ...
    .option('--router-file <name>', 'Router file name (default: fastify-router.gen.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routerFile = options.routerFile ?? 'fastify-router.gen.ts'
      ...
      try {
        ...
        // Generate routes
        const generator = new FastifyRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
