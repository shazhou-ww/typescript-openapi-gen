...
import { ElysiaRouteGenerator } from '../../../lib/route-generator'

function registerRouterElysiaCommand(program: Command) {
  program
    .command('elysia')
    .description('Generate Elysia router decorator from OpenAPI specification')
    ...
    .option('--router-file <name>', 'Router file name (default: elysia-router.gen.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routerFile = options.routerFile ?? 'elysia-router.gen.ts'
      ...
      try {
        ...
        // Generate routes
        const generator = new ElysiaRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
