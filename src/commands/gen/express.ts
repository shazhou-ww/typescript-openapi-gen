...
import { ExpressRouteGenerator } from '../../lib/route-generator/index'

export default class GenExpress extends Command {
  ...
  static override description =
    'Generate Express app (controllers + routes) from OpenAPI specification'
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(GenExpress)
    ...
    try {
      ...
      const routeGenerator = new ExpressRouteGenerator(
        openApiDoc,
        controllerPath,
        outputPath,
        { prettierConfig },
