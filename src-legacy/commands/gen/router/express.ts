...
function registerRouterExpressCommand(program: Command) {
  program
    .command('express')
    .description('Generate Express router from OpenAPI specification')
    ...
    .option('--router-file <name>', 'Router file name (default: express-router.gen.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routerFile = options.routerFile ?? 'express-router.gen.ts'
      ...
      try {
        ...
        // Generate routes
        const generator = new ExpressRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
