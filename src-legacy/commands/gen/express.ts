...
import { ExpressRouteGenerator } from '../../lib/route-generator'

function registerExpressCommand(program: Command) {
  program
    .command('express')
    .description('Generate Express app (controllers + routes) from OpenAPI specification')
    ...
    .option('--route-file <name>', 'Route file name (default: express-routes.ts)')
    ...
    .action(async (file: string, options: {
      ...
      const routeFile = options.routeFile ?? 'express-routes.ts'
      ...
      try {
        ...
        const routeGenerator = new ExpressRouteGenerator(openApiDoc, controllerPath, outputPath, {
          prettierConfig,
        })
